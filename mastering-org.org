...
** Property
  ...
  - PROPERTY (~:PROPERTY:value~) :: A property is a key-value pair associated with an entry.
       ...
       with every entry in a file (see ~org-use-property-inheritance~).  Like a tag, a property is
       delimited on both ends by colons, so this is a property: ~:A_PROPERTY:Some value~.  The key
       portion is case-insensitive.  A property can have only one entry per property drawer.

    - Define /allowed values/ for a property by setting a property =SOMETHING_ALL:value1 value2=
      for it.  It is inherited.  The values are space-delimited list entries.  To create a
      globally-inheritable property, use =#+PROPERTY:Something_ALL value1 value=; or set the
      variable ~org-global-propertiers~.
...
- =org-confirm-babel-evaluate= ::
     ...
     prompts before code evaluation.
