...
** Agenda
  ...
  - Agenda :: Org can select items based on various criteria and display them in a separate
    ...
    + Agenda Dispatcher :: The views are created through a dispatcher, which should be bound to a
         [[*Global Key Bindings][global key]]---for example ~C-c a~.  After pressing ~C-c a~, an additional letter is required
...
** Global Key Bindings
   ...
   Three Org commands should be accessible anywhere in Emacs (not just in Org); they should be
   bound to GLOBALLY AVAILABLE KEYS (reserved for users): see [[info:elisp#Key%20Binding%20Conventions][info:elisp#Key Binding Conventions]]
   #+begin_quote
   Don’t define ‘C-c LETTER’ as a key in Lisp programs.  Sequences consisting of ‘C-c’ and a letter
   (either upper or lower case) are reserved for users; they are the *only* sequences reserved for
...
>>>>>>> 9fecd2510bcc615810ebfee7dee9985d1ec2f47a
